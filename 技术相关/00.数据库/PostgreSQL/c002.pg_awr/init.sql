...
begin
  ...
  loop
    perform __rds_pg_stats__.snap_delete_data(v_snap_id::int8);
  end loop;
...
begin
  ...
  loop
    perform __rds_pg_stats__.snap_delete_data(v_snap_id);
  end loop;
...
begin
  ...
  for v_snap_id in select id from snap_list where id <= (select id from snap_list order by id desc limit 1 offset i_reserved) order by id 
  loop
    perform __rds_pg_stats__.snap_delete_data(v_snap_id);
  end loop;
